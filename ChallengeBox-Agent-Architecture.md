...
- `validate(args)` — a pure predicate over the same arguments as `reference`: `True` if the input satisfies every precondition the statement states, `False` otherwise. Every input `gen` and the STRESS edge-case list produce is checked here before `reference` computes an expected output for it (§5.5) — this is what stops a differential run from comparing candidate against reference on input the statement itself calls invalid (e.g. repeated identifiers where the statement says distinct, or an operation naming an identifier that was never created). A validator that cannot decide should return `True` rather than guess. The prompt also requires the two to be written from *one* precondition list — `validate` from it first, then `gen` from the same list, with one `gen` output traced through `validate` by hand before answering — and warns about the specific failure §5.3.1 reacts to: a stateful `validate` that forgets to record what an earlier operation created and then rejects every later reference to it. Every input the gate uses goes through it, not just the generated tiers: `gen_max`'s max-size output is checked before it becomes the stress input (a rejection falls back to the degraded medium case rather than gating on an input the statement forbids), and so is every input the shrinker invents (§5.6). `verify._validate_inputs` is the single place that call is made; a validate that crashed or timed out on an input gives no verdict, which is not a rejection, and a validate that rejected a whole tier is distrusted exactly as before. When a tier is distrusted AND `reference()` returned the same answer for inputs that differ, those are two independent signals that the oracle cannot parse this input format at all: the tier is emptied with a note rather than used to fail a candidate for printing the right answer.
