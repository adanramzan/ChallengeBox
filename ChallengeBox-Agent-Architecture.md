...
**The afford thresholds follow the configured model, not a literal.** `[limits] oracle_retry_afford_s` and `oracle_selfrepair_afford_s` were tuned to a fast model measured at up to 128 s per call and sat at 130 s; against qwen3-coder (12–45 s per oracle call) that refuses every optional oracle call through the whole second half of a deadline. `solve.oracle_afford_s` takes the smaller of the config value and the fast role's own measured ceiling (`timeout_cap_s × 0.5 + 30`, the call at half its cap plus the time to use what comes back), so swapping the model re-tunes the threshold by itself; the config values are now 60.0 and act as a ceiling, never a floor. During the overlap the remaining budget is nearly the whole deadline, so this number only ever binds a late trigger. The cost cap applies to every trigger unchanged.
...
**Which prompt goes to which role is config too.** `config.toml`'s top-level `[roles]` table maps prompt name → role (`solve = "strong"`, `oracle = "fast"`, `stress = "strong"`, `repair = "strong"`), read once through `Run.role()`; a config without the table gets `solve.PROMPT_ROLES`, the mapping the system was tuned on before it existed (`stress` → fast). The shipped mapping sends STRESS to the strong role because the STRESS artefact was the primary cause in three of three Opus-era runs of one focus problem — an input nested 3 333× over the stated cap, a 7-operation input whose first operation was invalid (twice), and eight mis-shaped `EDGES` — and because it is the one generation call that is *off the critical path*: preparation overlaps the solve call (§5.3), so a slower, better author costs no wall time at all. It runs at the cheaper effort its `tag_extra` entry selects, and its cap stays `generate_call_share` like the oracle's.
