...
The three generation calls run concurrently, and preparation of the gate's inputs overlaps them: the orchestrator waits for the ORACLE reply first and starts building the small and medium differential tiers immediately, while the SOLVE and STRESS calls are still in flight (§5.3). Each SOLVE reply is then built into a candidate and **gated as it lands**, while the other attempt is still running (§5.7). The gate is deterministic and costs no tokens. The repair loop runs at most twice against a semantic (behavioral) failure and, independently, at most twice against a mechanical (static/compile) one — see §5.7. The time controller can interrupt at any point and finalize.
