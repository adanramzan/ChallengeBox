...
**Which models.** The strong role is a *thinking* model (`anthropic/claude-opus-5` through OpenRouter, `reasoning.effort = "medium"`, `max_tokens = 24000`) and the fast role a literal coder model (`qwen/qwen3-coder`). Rounds 10–14 settled why. On 5cb294c18288 the coder model in the strong role read the *semantics* correctly — the emitted candidate matched an independent reference on 8 366 differential cases and six hand traces — and never once read the *complexity*: four candidates for that statement (two independent solves, two repairs) all materialized a layout the statement calls "enormous" and all iterated a count it bounds by 10^18, so the emitted solution needs ~10^11 s at the stated limits. No prompt or gate change reaches that; connecting "enormous" to "do not build the list" is the reasoning the strong role exists to buy. The earlier finding that no reasoning model returns inside a 300 s deadline was measured with `max_tokens = 12000`: reasoning tokens count against `max_tokens`, so the thinking spent the whole budget before any `===CODE===` block was written and the call returned nothing. The budget now covers thinking *and* answer. The fast role still ships `qwen/qwen3-coder`, but the shipped `[roles]` mapping no longer sends it anything (see below): returning in 17–24 s turned out not to be the same as reading the statement, and the oracle it wrote was what ended bench19 and bench20. Oracle/candidate independence — the reason the two roles were put on different models after a round where both ran the same one — is now carried by generation context rather than by model identity: the oracle is written in its own call, from the statement alone, and never sees the candidate. `[limits] max_cost_usd_per_problem` is 1.50 accordingly: at $5/$25 per 1M the cap must bound a runaway, not stop the second repair of an ordinary run.

**Which prompt goes to which role is config too.** `config.toml`'s top-level `[roles]` table maps prompt name → role, read once through `Run.role()`; a config without the table gets `solve.PROMPT_ROLES`, the mapping the system was tuned on before it existed (`oracle`/`stress` → fast). The shipped mapping now sends **all four prompts to the strong role**, each measuring call at the cheapest effort its `tag_extra` entry offers, and each for the same reason: they are *off the critical path* (preparation overlaps the solve call, §5.3), so a slower, better author costs no wall time, and each of them was the run-ending fault in an Opus-era benchmark.

STRESS moved first, after the STRESS artefact was the primary cause in three of three Opus-era runs of one focus problem — an input nested 3 333× over the stated cap, a 7-operation input whose first operation was invalid (twice), and eight mis-shaped `EDGES`. It worked: bench19's and bench20's Opus-authored stress modules were valid, inside every stated bound, and checked their own bounds in code before returning.

ORACLE follows on the same evidence pointing the other way. The qwen-authored oracle was the primary or the only remaining cause of an unverified run in both bench19 and bench20: a `reference()` that rejected half of what its own `gen()` produced, 192 of 200 small answers identical because `gen()` invented identifiers instead of drawing them from the state it had just built (so a 200-case tier asserted one fact), and a regenerated replacement that came back unusable. The oracle is the ground truth of the entire system — every differential tier is worth exactly what the reference is worth — and no prompt or gate change reaches a reference with no code path for half the statement. Two costs, both accepted: the oracle call now competes with SOLVE and STRESS for the strong role's `max_concurrent = 3`, which is exactly three calls, so the three still start together and none queues; and preparation now begins at the oracle's latency (expected 60–90 s at low effort, against qwen's ~18 s) rather than almost immediately, which still lands well inside the solve call's measured 165–240 s, so the tiers are built before the candidate arrives exactly as before.
