...
**The thinking budget follows the call's timeout.** A fixed effort level is the wrong knob, because it is a statement about the *model* while the thing that actually binds is the *deadline*: medium effort overran the strong role's 240 s cap in 3 of 10 runs while low fit easily and sometimes underthought, and which of the two a given statement needs is not knowable before the call. What *is* known when a call is made is its own `timeout_s` — already the deadline's share for this phase — and at a measured output rate that converts directly into tokens. So the optional per-role `reasoning_budget = { tokens_per_s, answer_reserve_s, min_tokens, max_tokens }` replaces the effort: `llm.chat` computes `N = clamp(tokens_per_s × (timeout_s − answer_reserve_s), min_tokens, max_tokens)` for that one call and sends `reasoning = { max_tokens = N }`, overriding any `effort` the role's `extra` carries, and records `reasoning_max_tokens` in the call record beside the tokens actually spent. `tokens_per_s` is a measured latency constant of the model and does **not** scale with the deadline (Opus runs ~70–80 tok/s including reasoning — bench22: 8 805 completion tokens in 107.7 s; medium-effort runs 13–16k tokens in 152–240 s — and the config sets 60 for headroom); `answer_reserve_s` is the code that still has to be written once the thinking stops, which is the same reason the SOLVE prompt asks for CODE first. The clamp keeps a short deadline from buying no thinking and a long one from buying more than the model will use. Post-gate calls need no special case any more: a repair's timeout is a fraction of a solve's, so its N is too, and the OpenRouter strong role's `repair`/`solve_fresh` `tag_extra` entries are gone. `Role.tag_extra` (a TOML table per role, keyed by prompt tag, merged over `Role.extra` for that tag's request only) remains for a tag that must pin a *shape* rather than an amount — an explicit `reasoning` there wins over the budget — and the tag plumbing is what a fresh solve's own tag (`solve_fresh`) rides on through `Run.chat`, the log, `report.json`'s `calls` and the `FakeLLM` script. Which knob a provider takes stays in role config, never in a branch in `llm.py`: the fast role keeps `effort = "low"` by simply not setting a budget.
