...
**Post-gate calls think less.** `Role.tag_extra` (a TOML table per role, keyed by prompt tag) is merged over `Role.extra` for that tag's request only, and `config.toml` sets `reasoning = { effort = "low" }` under `repair` and `solve_fresh` for the OpenRouter strong role. The initial solve was medium until the 239–240 s cutoffs described in §8.1; it is now `low` too, so `tag_extra` no longer changes the effort, and it stays in place because the tag plumbing is what any future per-tag knob rides on. After the gate the deadline is measured in tens of seconds, not hundreds. Which knob a provider takes stays in role config, never in a branch in `llm.py`, which only knows that a tag may carry its own extra; a fresh solve therefore carries its own tag (`solve_fresh`) through `Run.chat`, the log, `report.json`'s `calls` and the `FakeLLM` script. It is now the only use of `tag_extra`: the ORACLE and STRESS calls take their effort from the fast role's own `extra` instead — see §8.1.
